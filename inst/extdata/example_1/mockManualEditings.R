#' Edit Data Dictionary for Manual Configuration
#'
#' This function modifies the data dictionary in the specified Excel workbook based on the provided project configuration.
#' It updates the data files and the target columns in the data dictionary according to the specified rules.
#'
#' @param projectConfiguration A ProjectConfiguration object containing project configuration settings, including:
#'   - dataImporterConfigurationFile: Path to the Excel file containing the data importer configuration.
#' @return NULL This function modifies the Excel workbook in place and does not return a value.
#' @export
#' @examples
...
mockManualEditings.DataDictionary <- function(projectConfiguration) {
  ...
  dtDataFiles <- rbind(
    dtDataFiles,
    data.table(
      dataFile = file.path("..", "..", "Data", "observedData_drugX.csv"),
  ...
  # Update source columns based on target columns
...
#' Edit Population Settings
#'
#' This function updates the virtual twin population settings in the specified Excel workbook based on the provided project configuration.
#' It modifies the population name and adds a random population based on observed data.
#'
#' @param projectConfiguration A ProjectConfiguration object containing project configuration settings, including:
#'   - individualsFile: Path to the Excel file containing individual configurations.
#'   - populationsFile: Path to the Excel file containing population configurations.
#' @param dataObserved A data table containing observed data for individuals.
#' @param tutorialstep An integer indicating the tutorial step for the operation.
#' @return NULL This function modifies the Excel workbook in place and does not return a value.
#' @export
#' @examples
#' mockManualEditings.Population(projectConfiguration, dataObserved, tutorialstep)
...
#'
#' This function adds or updates scenario configurations in the specified Excel workbook based on the provided project configuration.
#' It modifies scenarios for individual and virtual twin populations based on the tutorial step.
#'
#' @param projectConfiguration A ProjectConfiguration object containing project configuration settings, including:
#'   - modelParamsFile: Path to the Excel file containing model parameters.
#'   - individualsFile: Path to the Excel file containing individual configurations.
#'   - scenariosFile: Path to the Excel file containing scenario configurations.
#' @param dataObserved A data table containing observed data for individuals.
#' @param tutorialstep An integer indicating the tutorial step for the operation.
#' @return NULL This function modifies the Excel workbook in place and does not return a value.
#' @export
...
mockManualEditings.Scenario <- function(projectConfiguration, dataObserved, tutorialstep) {
  if (tutorialstep == 4) {
    # add a sheet with parameters to adjust for a reference population
    wb <- openxlsx::loadWorkbook(projectConfiguration$modelParamsFile)
    dtPar <- xlsxReadData(wb = wb, sheetName = "Template")
    # delete template rows
    dtPar <- dtPar[FALSE]
    dtPar <- rbind(
      dtPar,
      data.table(
        "container Path" = "CYP3A4",
  ...
  if (tutorialstep == 2) {
    ...
    dtScenario <- rbind(dtScenario,
                        data.table(
                          scenario_name = "virtual_twin_population_iv",
                          populationId = "virtual_twin_population",
                          readPopulationFromCSV = TRUE,
                          modelFile = "iv 1 mg (5 min).pkml"
                        ),
                        data.table(
                          scenario_name = "virtual_twin_population_po",
                          populationId = "virtual_twin_population",
                          readPopulationFromCSV = TRUE,
  ...
  if (tutorialstep == 3) {
    dtScenario <- rbind(dtScenario,
                        data.table(
                          scenario_name = "random_population_iv",
                          populationId = "random_population",
                          readPopulationFromCSV = TRUE,
                          modelFile = "iv 1 mg (5 min).pkml"
                        ),
                        data.table(
                          scenario_name = "random_population_po",
                          populationId = "random_population",
                          readPopulationFromCSV = TRUE,
                          modelFile = "po 3 mg (solution).pkml"
                        ),
  ...
  if (tutorialstep == 4) {
    dtScenario <- rbind(dtScenario,
                        data.table(
                          scenario_name = "reference_population_iv",
                          populationId = "random_population",
                        ...
                        data.table(
                          scenario_name = "reference_population_po",
                          populationId = "random_population",
                          modelParameterSheets = "CYP3A4_2",
                          readPopulationFromCSV = TRUE,
                          modelFile = "po 3 mg (solution).pkml"
...
#' Edit Data Groups Configurations
#'
#' This function updates data group configurations in the specified Excel workbook based on the provided project configuration.
#' It modifies the data groups and output paths based on the tutorial step.
#'
#' @param projectConfiguration A ProjectConfiguration object containing project configuration settings, including:
#'   - plotsFile: Path to the Excel file containing plot configurations.
#' @param dataObserved A data table containing observed data for individuals.
#' @param tutorialstep An integer indicating the tutorial step for the operation.
#' @return NULL This function modifies the Excel workbook in place and does not return a value.
#' @export
#' @examples
#' mockManualEditings.DataGroups(projectConfiguration, dataObserved, tutorialstep)
...
#' Edit Time Plot Configurations
#'
#' This function updates time plot configurations in the specified Excel workbook based on the provided project configuration.
#' It modifies plot settings for different tutorial steps, including adding time ranges and updating plot details.
#'
#' @param projectConfiguration A ProjectConfiguration object containing project configuration settings, including:
#'   - plotsFile: Path to the Excel file containing plot configurations.
#' @param dataObserved A data table containing observed data for individuals.
...
#' @export
#' @examples
#' mockManualEditings.TimePlot(projectConfiguration, dataObserved, tutorialstep)

mockManualEditings.TimePlot <- function(projectConfiguration, dataObserved, tutorialstep) {
  # helper function: adds a header line before specified PlotName
  ...
  # helper function: copies spcified PlotName
  ...
  # helper function: shifts Line to end of table
  ...
  # Plotconfig
  # Line one is a header line with additional information to help the input and is not edited
  ...
  if (tutorialstep == 1) {
    ...
    dtPlots[plotName == "individual_1_iv", plotCaptionAddon := "DrugX was administered as a 1mg Iv application with an infusion time of 5 minutes."]
    ...
    dtPlots[plotName == "virtual_twin_population_iv", plotCaptionAddon := "DrugX was administered as a 1mg Iv application with an infusion time of 5 minutes."]
    dtPlots[plotName == "virtual_twin_population_po", plotCaptionAddon := "DrugX was administered as a 5mg oral application of a solution."]
    ...
    dtPlots[plotName %in% c("virtual_twin_population_iv-1", "virtual_twin_population_iv-2"), scenarioLongName := "individual simulations of study 000"]
    ...
    dtPlots[plotName == "virtual_twin_population_po", scenarioLongName := "individual simulations of study 000"]
    dtPlots[plotName == "virtual_twin_population_po", facetType := "Scenario vs Output"]

    # add goodness of fit plot in new line
    ...
    dtPlots[
      plotName %in% c("reference_population_iv", "reference_population_po"),
      plotName := "reference"
    ]
    # shift reference scenarios in column referenceScenario and add control scenarios
