...
vignette: >
  %\VignetteIndexEntry{PK_Parameter}
...
PK-parameter calculation extends `{esqlabsR}` workflows. The configuration table `PKParameter.xlsx` is available via `projectConfiguration$addOns$pKParameterFile`.
...
When calling `runOrLoadScenarios()` or `runAndSaveScenarios()`, selected PK parameters are automatically calculated and saved to the `PKAnalysisResults` output folder.
If simulation results already exist and you want to recalculate PK parameters without rerunning simulations, call `calculatePKParameterForScenarios()` explicitly:

```{r calculatePKParameterForScenarios}
...
If available, import observed PK data as a data.table so it can be compared against simulated PK parameters.
In dictionary-based imports, PK data classes are represented by `DATACLASS$pkIndividual` and `DATACLASS$pkAggregated`.
The full data import workflow is described in the vignette `DataImport_by_Dictionary`.
