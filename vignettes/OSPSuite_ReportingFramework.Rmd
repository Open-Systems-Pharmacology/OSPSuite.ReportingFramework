...
### Access via RStudio Addin

You can also access the workflow template through an RStudio addin. 
To use the addin, simply navigate to the Addins menu in RStudio, and select "Open Template Workflow". This will execute the `openWorkflowTemplate()` function and open the template in a new document.
...
After setting this option, calling `openWorkflowTemplate()` or using the addin will open your customized template instead of the default one.
...
# Integration with esqlabsR Package
...
To facilitate the creation of custom plot functions, you can open a template using the RStudio addin. Navigate to the Addins menu in RStudio and select "Open Template Plotfunction". Alternatively, you can use the following function to open the template directly:
