...
vignette: >
  %\VignetteIndexEntry{Electronic_Package}
...
The `ePackage` workflow exports the files required to reproduce simulation and reporting outputs in a structured folder. It is intended for transparent handover and reproducible execution of workflow scripts.
...
The electronic package folder is part of the project configuration and is available as `projectConfiguration$addOns$electronicPackageFolder`. You can overwrite this default path:
...
Provide a unique `wfIdentifier` for each exported workflow in the same project and the list of scenarios to include. See `? exportSimulationWorkflowToEPackage` for details.

2. **Running Simulation Workflows**

Within the electronic package folder, a workflow file will now exist. For `wfIdentifier = 1`, the file is `w1_workflow_r.txt`.
...
- **importWorkflow**: The script calls `importWorkflow(projectDirectory, wfIdentifier, ePackageFolder, configurationDirectory)` to build a workflow-specific project structure with all required files. The output folder name is kept from your original project configuration, while other folders are generated with suffix `_w<wfIdentifier>` to avoid overwriting existing configuration tables.
