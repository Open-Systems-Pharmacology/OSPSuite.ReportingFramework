...
- `dataClassType`: Data class to import (`"timeprofile"` or `"pkParameter"`).
...
`DATACLASS` provides standard values for `DataClass` (for example `DATACLASS$tpIndividual`, `DATACLASS$tpAggregated`, `DATACLASS$pkIndividual`, `DATACLASS$pkAggregated`) and should be used in scripts to avoid typos.
