...
- **Data Types**: The package supports importing observed time profiles, pharmacokinetic (PK) parameters, and scalar numeric values.

- **File Format**: Time profile and PK parameter source files are read from CSV tables. Scalar numeric values can be read from a dedicated sheet in the same Excel configuration workbook.
...
- **Output**: `readObservedDataByDictionary` returns one data.table per call, based on `dataClassType`.
  - Use `dataClassType = "timeprofile"` to import observed time profile data.
  - Use `dataClassType = "pkParameter"` to import observed PK parameter data.
  - Use `dataClassType = "numericValues"` to import scalar numeric values.
...
The `readObservedDataByDictionary` function reads and processes observed data based on the project configuration and data dictionary. Use these arguments to control import behavior:

- `projectConfiguration`: Project configuration object.
- `spreadData`: If `TRUE`, imported metadata is propagated to configuration tables.
- `dataClassType`: Data class to import (`"timeprofile"`, `"pkParameter"`, or `"numericValues"`).
- `fileIds`: Optional subset of file identifiers from the `DataFiles` sheet.

Typical usage pattern:

```{r readObservedDataByDictionary, eval = FALSE, echo = TRUE}
# Import observed time profiles
...
# Import observed PK parameters
dataObservedPK <- readObservedDataByDictionary(
  projectConfiguration = projectConfiguration,
...
In most workflows, `dataObserved` and `dataObservedPK` are imported with separate calls as shown above.

## AI-Assisted Data Import Setup

To prepare data-import requests for an AI assistant, generate a task-specific prompt that includes your current project context:
...
Before applying AI-suggested changes, validate the configuration:
...
RStudio Addin shortcut for this prompt:
...
`DATACLASS` provides standard values for `DataClass` (for example `DATACLASS$tpIndividual`, `DATACLASS$tpAggregated`, `DATACLASS$pkIndividual`, `DATACLASS$pkAggregated`, `DATACLASS$numericValues`) and should be used in scripts to avoid typos.
...
Use this sheet when numeric values should be maintained in configuration instead of hard-coded in scripts.

- Required columns in the sheet: `VariableName`, `Value`, `Unit`, `Reference`, `Description`.
- Returned table columns after import: `variableName`, `value`, `unit`, `reference`.
- The `Description` column is metadata and is not returned.
- Variable names must be unique across imported numeric-value sheets.

Add a row in `DataFiles` with `DataClass = DATACLASS$numericValues` and `Dictionary = "NumericValues"` (or another sheet name if configured).

```{r echo=FALSE, eval=TRUE}
numericValuesSheet <- data.table(
  VariableName = c("Dose", "InfusionDuration", "LowerLimit"),
  Value = c(100, 1.5, 0.05),
  Unit = c("mg", "h", "mg/L"),
  Reference = c("Study 1234", "Protocol", "Bioanalytical report"),
  Description = c(
    "Single dose amount",
    "Infusion duration used in protocol",
    "Lower quantification limit"
  )
)

knitr::kable(numericValuesSheet)
```

Retrieve one scalar with optional unit validation:

```{r eval=FALSE}
dose <- getNumericValue(numericValues, "Dose", expectedUnit = "mg")
halfLife <- getNumericValue(numericValues, "InfusionDuration", expectedUnit = "h")
```
...
```{r, echo=FALSE}
tpDictionaryInd <- tpDictionary[!(TargetColumn %in% c("yErrorValues", "yErrorType", "nBelowLLOQ", "numberOfIndividuals", "yMin", "yMax", "route"))]
