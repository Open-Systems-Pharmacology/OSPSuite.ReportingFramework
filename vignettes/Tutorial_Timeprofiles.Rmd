...
projectConfiguration <- pBuilder$iniTestProject(rootDirectory = NULL)
options(OSPSuite.RF.verbose = FALSE)

rootDirectory <- fs::path_common(path = c(
...
Execute the `runPlot` call again to generate the plots. The variable `plotList` contains all generated plots. In the example below, the input argument `xScaleArgs = c(NA, NA)` is added. This argument is passed to the function `ospsuite.plots::plotTimeprofile` and overwrites the default time-profile x-limits, which is `c(0, NA)`.
