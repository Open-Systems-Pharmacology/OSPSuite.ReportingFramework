...
invisible(file.copy(
  from = file.path(instDirectory, "Tutorial", "adults.csv"),
...
modelFiles <- list.files(file.path(instDirectory, "Tutorial"), pattern = "\\.(pkml|PKML)$", full.names = TRUE)
...
```
The models used in the tutorial ("iv_1_mg_5_min.pkml" and "po_3_mg_solution.pkml") are included in the Tutorial materials.
...
2. Call `mergeQmds()` to concatenate the individual `.qmd` fragments into one document, then `renderWord()` to produce the final `.docx`.
...
The first step is to run all plots without a `plotNames` filter so every configured plot is generated and saved. The second step merges the individual `.Qmd` fragments into a single document and renders it to Word.
