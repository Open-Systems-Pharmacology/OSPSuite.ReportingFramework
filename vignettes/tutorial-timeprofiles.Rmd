...
# Open the document
shell.exec(filePath)  # For Windows
# system(paste("open", shQuote(filePath)))  # For macOS
# system(paste("xdg-open", shQuote(filePath)))  # For Linux
```
...
The paths should be relative to th configuration table which is in '<Rootdirectory>/Scripts/ReportingFramework', the data are saved in '<Rootdirectory>/Data' so the correct path is '../../Data'


```{r, echo=FALSE, eval = TRUE}

tmp = xlsxReadData(wb = system.file(
...
- For 'subjectId' and 'individualId' we can use the columns 'SID', as we have only one study, subjecId is already unique
...
- The biometrics of the individuals in the observed data are added to the "Individual.xlsx".
...
dtScenarios = xlsxReadData(wb = system.file(
...
For the plot configuration we call a helper function which adds all defined scenarios with default settings to the plot configuration table. As this is a helper function, which changes the inputc configuration of a workflow, it is not intended to be part of an final workflow and will throw an error if run as validRun.
...
- Edit the columns:
  ...
  - DataGroupIds: data group identifier whcih corresponds to this scenario
...
dtPlots = xlsxReadData(wb = system.file(
  "extdata","example_1",'Plots.xlsx',
  package = "ospsuite.reportingframework",
  mustWork = TRUE),
  sheetName = 'TimeProfiles',
...
All figures and captions are filed in "Outputs\ReportingFramework\TimeProfiles" as png and textfiles.
...
Go to the Export populations of the workflow and add and execute following lines:
...
The model file is used for unit conversions. As our population contains only parameters not related to the application, we could have used both available model files, the one for the oral administration and the iv administration file.
...
To omit a rerun from previously defined scenarios, we change in the scenarioList initialisation the parameter `scenarioNames` from `NULL` to the names of the newly created scenarios.
...
As this will lead to very large plot with many rwos, the plot is splitted in different plots. The maximal row numer is an input parameter of the plot function. 
...
- update the `PlotName` for both 'random_population' sceanrios to 'random_population'
...
- add the corresonding random poulation scenarios to column 'Scenario'
...
- Add new columns 'TimeRange_h0_6' and 'TimeRange_h6_24' for the newly created timeranges
