...
- This alert serves to inform users that the estimates may not be reliable due to insufficient sample sizes or high variability in the data, prompting them to consider re-evaluating the simulations or increasing the sample size for improved precision.
...
The ratio calculation distinguishes between scenarios with the same populations and those with different populations. If the scenarios being compared are based on the same populations, the plots will show summary statistics of individual ratios. In contrast, for scenarios based on different populations, the ratios are calculated using summary statistics derived from bootstrapping methods, providing a more robust estimate of variability.
