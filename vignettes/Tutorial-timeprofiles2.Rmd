...
```{r setup, echo=FALSE, eval=TRUE}
 ...
 modelFiles <-
   list.files(file.path(instDirectory, 'Models'))
 names(modelFiles) <-
   substr(modelFiles, 1, 2)

 projectConfiguration <-
   ospsuite.reportingframework:::iniTestProject(rootDirectory = NULL,
                                                verbose = FALSE,
                                                modelFiles = modelFiles)

 rootDirectory <-
   fs::path_abs(file.path('..', '..'), start = projectConfiguration$configurationsFolder)
 ...
 invisible(lapply(configFiles, function(f) {
   file.copy(from = f, to = file.path(projectConfiguration$configurationsFolder, basename(f)), overwrite = TRUE)
 }))
 ...
 invisible(lapply(dataFiles, function(f) {
   file.copy(from = f, to = file.path(rootDirectory, 'Data', basename(f)), overwrite = TRUE)
 }))

 invisible(file.copy(from = file.path(instDirectory, 'Populations', 'adults.csv'), to = file.path(projectConfiguration$populationsFolder, 'random_population.csv'), overwrite = TRUE))
```
...
```{r tutorial-path}
print(system.file("extdata", "Tutorial", package = "ospsuite.reportingframework", mustWork = TRUE))
```

The models used in the tutorial (`iv 1 mg (5 min).pkml` and `po 3 mg (solution).pkml`) are available here:

```{r model-path}
print(system.file("extdata", "Models", package = "ospsuite.reportingframework", mustWork = TRUE))
```
...
- Set up your project directory (`<rootDirectory>`) which should be empty.
- Create a subfolder `Scripts/ReportingFramework` (`<configurationDirectory>`).
...
# Set to TRUE for a final valid run.
...
# tmp <-
#   xlsxReadData(wb = projectConfiguration$dataImporterConfigurationFile,
#                 sheetName = "DataFiles",
...
- For biometrics columns, use the source columns 'Age', 'WGHT0', 'HGHT0', and 'SEC'.
...
- The identifier for the data groups and outputs are added to the `Plots.xlsx`, sheets 'DataGroups' and 'Outputs'. These tables configure properties for output plots such as 'DisplayName' and 'DisplayUnit'. Please add the 'OutputPaths' for the 'OutputPathsIds'.
...
(See the vignette for the `ospsuite.reportingframework` package, topic 'Population').
...
Do not forget to edit the columns `PlotCaptionAddon`, `TimeRange_firstApplication`, and `TimeRange_lastApplication`. Add subsections.
...
dataObserved <-
  readObservedDataByDictionary(projectConfiguration = projectConfiguration)
...
```{r randompopulation-config, echo=FALSE, eval=TRUE}
tmp <- xlsxReadData(
...
This will create a population based on the configuration table from the previous section. It is saved as a CSV file in `Models`.

### Step 4: Set Up and Run Scenario

Add two more scenarios to the `Scenarios` sheet:

```{r scenario-randompopulation, echo=FALSE, eval=TRUE}
knitr::kable(dtScenarios)
```

Reload all scenarios:

```{r simulate-random-populations, eval=FALSE}
# scenarioList <-
#   createScenarios.wrapped(
#     projectConfiguration = projectConfiguration,
#     scenarioNames = NULL,
...
Do not forget to edit the columns `TimeRange_firstApplication`, `TimeRange_lastApplication`, and add `longNames` in the sheet `Scenario`. Add subsections.
...
Do not forget to edit the columns `DataGroupIds`, `PlotCaptionAddon`, `TimeRange_firstApplication`, and `TimeRange_lastApplication`. Add subsections.
