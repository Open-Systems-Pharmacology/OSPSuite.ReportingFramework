...
If you do not use `readObservedDataByDictionary` for data import, you can add the sheet by calling `setupVirtualTwinPopConfig(projectConfiguration, dataObserved)`, which configures the virtual twin population based on the observed data.
...
The function `exportVirtualTwinPopulations` is called to export this population to a CSV table in `<rootdirectory>/Models/Populations`. It includes an additional column, 'ObservedIndividualId', which contains the individual identifier which relates to the observed data.
