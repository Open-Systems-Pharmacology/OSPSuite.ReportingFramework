...
When you load a configuration, both versions are checked against the installed packages.
If a version is missing or does not match, you are prompted in an interactive session to update the file and continue; in non-interactive runs (for example CI), loading stops until the version is updated.
New projects created with `initProject()` receive the current framework version automatically.
Saving the configuration with `projectConfiguration$save()` updates both stored versions.
