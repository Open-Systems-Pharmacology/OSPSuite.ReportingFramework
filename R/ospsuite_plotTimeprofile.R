#' @importFrom ospsuite addResidualColumn
NULL
...
#' @param mapping additional mappings specified by user
#' @param observedMapping Aesthetic mappings for the observed data, typically created with `ggplot2::aes()`.
...
#' ))
#' }
ospsuite_plotTimeProfile <- function(
  plotData, # nolint
  metaData = NULL,
  mapping = ggplot2::aes(),
...
#' ))
#' }
ospsuite_plotPredictedVsObserved <- function(
  plotData, # nolint
  metaData = NULL,
  mapping = ggplot2::aes(),
  ...
...
#' Plot Residuals vs a Covariate
#'
#' Plots residuals against time, observed values, or predicted values. Residuals
#' are computed via `ospsuite::addResidualColumn` using the specified scaling,
#' aligning with the OSPSuite-R residual pipeline.
#'
#' @param plotData A data.table with a `predicted` column (interpolated simulated
#'   values at observed time points) produced by `addPredictedValues`.
#' @param residualScale Scaling method for residuals: `"log"` (default), `"linear"`,
#'   or `"ratio"`. Passed to `ospsuite::addResidualColumn`.
#' @param xAxis Which quantity to place on the x-axis: `"time"` (`xValues`),
#'   `"observed"` (`yValues`), or `"predicted"`.
#' @param metaData A list with plot metadata. Constructed from data if NULL.
#' @param mapping Additional ggplot2 aesthetic mappings.
#' @param ... Additional arguments passed to `ospsuite.plots::plotResVsCov`.
#'
#' @return A `ggplot2` plot object representing residuals vs the chosen covariate.
#' @export
#'
#' @examples \dontrun{
#' ospsuite_plotResidualsVsCovariate(plotData, residualScale = "log", xAxis = "time")
#' }
ospsuite_plotResidualsVsCovariate <- function(
  plotData, # nolint
  residualScale = "log",
  xAxis = c("time", "observed", "predicted"),
  metaData = NULL,
  mapping = ggplot2::aes(),
  ...
) {
  xValues <- yValues <- residualValues <- predicted <- group <- NULL
  ...
  plotData <- ospsuite::addResidualColumn(
    pairedData = plotData,
    observed = "yValues",
    predicted = "predicted",
  ...
  plotData <- plotData[!is.na(residualValues) & !is.nan(residualValues)]

  xMapping <- switch(
    xAxis,
    time = ggplot2::aes(x = xValues, y = residualValues),
    observed = ggplot2::aes(x = yValues, y = residualValues),
    predicted = ggplot2::aes(x = predicted, y = residualValues)
  )

  mapping <- structure(
    utils::modifyList(
      utils::modifyList(xMapping, ggplot2::aes(groupby = group)),
      mapping
    ),
    class = "uneval"
  )
  ...
  plotObject <- do.call(
    ...
    args = c(
      list(data = plotData, mapping = mapping, metaData = metaData),
      list(...)
    )
  )
...
#' @rdname ospsuite_plotResidualsVsCovariate
#' @export
#' @keywords internal
ospsuite_plotResidualsVsTime <- function(
  plotData, # nolint
  residualScale = "log",
  mapping = ggplot2::aes(),
  ...
) {
  .Deprecated("ospsuite_plotResidualsVsCovariate")
...
#' @export
#' @keywords internal
ospsuite_plotResidualsVsObserved <- function(
  plotData, # nolint
  residualScale = "log",
  mapping = ggplot2::aes(),
  ...
...
#' ))
#' }
ospsuite_plotResidualsAsHistogram <- function(
  plotData, # nolint
  metaData = NULL,
  residualScale = "log",
  mapping = ggplot2::aes(),
  ...
  mapping <- structure(
    utils::modifyList(
      ggplot2::aes(x = residualValues, groupby = group),
      mapping
    ),
    class = "uneval"
  )
...
#' ))
#' }
ospsuite_plotQuantileQuantilePlot <- function(
  plotData, # nolint
  metaData = NULL,
  residualScale = "log",
  mapping = ggplot2::aes(),
  ...
  mapping <- structure(
    utils::modifyList(
      ggplot2::aes(sample = residualValues, groupby = group),
      mapping
    ),
    class = "uneval"
  )
...
#' @param predictedIsNeeded If TRUE, the 'predicted' column is required; calculated from simulated data if absent.
#'        If FALSE, all data rows are returned.
#'
...
.validateAndConvertData <- function(plotData, predictedIsNeeded) {
  ...
  if (predictedIsNeeded) {
    if (!("predicted" %in% names(plotData))) {
      ...
      # predicted column already present; no recalculation needed
    }
  }
...
.constructMetDataForTimeProfile <- function(plotData) {
  ...
  if (length(xUnit) > 1) {
    stop(messages$errorospsuiteplotTimeprofileL1X())
  }
  ...
  if (length(yUnit) > 2) {
    stop(messages$errorospsuiteplotTimeprofileL1XX())
  }
  if ("yDimension" %in% names(plotData)) {
...
.getMappingForTimeprofiles <- function(plotData, metaData, userMapping) {
  ...
  if (any(names(metaData) %in% "y2")) {
    y2Unit <- metaData[["y2"]][["unit"]]
    mapping <- structure(
      c(mapping, ggplot2::aes(y2axis = yUnit == !!y2Unit)),
      class = "uneval"
    )
  }
