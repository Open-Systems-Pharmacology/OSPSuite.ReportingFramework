...
stampReportingFrameworkVersion <- function(path) {
  ...
  targetSheet <- if ("addons" %in% wb$sheet_names) "addons" else wb$sheet_names[1]
...
ProjectConfigurationRF <- R6::R6Class(
  ...
  private = list(
    ...
    .writeStandardConfigToXlsx = function(standardData, path) {
      openxlsx::write.xlsx(
        x = standardData[, c("Property", "Value", "Description"), drop = FALSE],
        file = path,
        overwrite = TRUE
      )
      invisible(NULL)
    ...
    .writeToConfigXlsx = function(propertyToSet, value, description) {
      ...
      if (!(targetSheet %in% wb$sheet_names)) {
        ...
        openxlsx::writeData(wb = wb, sheet = targetSheet, x = data.frame(
          Property = character(), Value = character(), Description = character(),
    ...
    .readAddOns = function() {
      ...
      inputData <- if ("addons" %in% wb$sheet_names) {
        xlsxReadData(wb = wb, sheetName = "addons", convertHeaders = FALSE, emptyAsNA = FALSE)
      } else {
    ...
    .writeRfVersionToPath = function(configPath) {
      ...
      if (!(REPORTING_FRAMEWORK_VERSION_PROPERTY %in% dtConfiguration$property)) {
        ...
        dtConfiguration[
          property == REPORTING_FRAMEWORK_VERSION_PROPERTY,
          `:=`(value = currentVersion, description = .reportingFrameworkVersionDescription())
        ]
      }
      xlsxWriteData(wb = wb, sheetName = targetSheet, dt = dtConfiguration)
      openxlsx::saveWorkbook(wb, configPath, overwrite = TRUE)
