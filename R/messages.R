...
messages$errorospsuiteplotTimeprofileL1 <- function(env = parent.frame()) {
    eval(quote("No data for this plot available"), envir = env)
}

messages$errorospsuiteplotTimeprofileL1X <- function(env = parent.frame()) {
    eval(quote("x Unit ambiguous"), envir = env)
}

messages$errorospsuiteplotTimeprofileL1XX <- function(env = parent.frame()) {
    eval(quote("y Unit ambiguous"), envir = env)
}

messages$errorospsuiteplotTimeprofileL1XXX <- function(env = parent.frame()) {
    eval(
        quote("Please do not mix different error Types in one plot"),
        envir = env
    )
}
