...
REPO_OWNER = "Open-Systems-Pharmacology"
REPO_NAME = "OSPSuite.ReportingFramework"
...
This task should be coordinated with the copy() audit and merge optimization for maximum impact.""",
        "labels": ["performance", "enhancement"]
    }
]
...
def create_issue(token, issue):
    ...
    try:
        # Get repository ID
        repo_data = json.dumps(repo_query).encode('utf-8')
        repo_req = Request(GITHUB_GRAPHQL_API, data=repo_data, headers=headers)
        with urlopen(repo_req) as response:
            repo_result = json.loads(response.read().decode('utf-8'))

        if "errors" in repo_result:
            raise Exception(f"Failed to get repository ID: {repo_result['errors']}")
        ...
        create_data = json.dumps(create_mutation).encode('utf-8')
        create_req = Request(GITHUB_GRAPHQL_API, data=create_data, headers=headers)
        with urlopen(create_req) as response:
            create_result = json.loads(response.read().decode('utf-8'))

        if "errors" in create_result:
            raise Exception(f"Failed to create issue: {create_result['errors']}")

        return create_result["data"]["createIssue"]["issue"]
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python3 create-performance-issues.py <GITHUB_TOKEN>", file=sys.stderr)
        print("\nPlease provide a GitHub Personal Access Token with repo scope.", file=sys.stderr)
        sys.exit(1)

    token = sys.argv[1]
    print(f"Creating {len(ISSUES)} performance optimization issues...\n")
    ...
    for issue in ISSUES:
        try:
            ...
            results.append({"error": str(e), "title": issue["title"]})

    print("\n=== Summary ===")
    successful = len([r for r in results if "error" not in r])
    failed = len([r for r in results if "error" in r])
    print(f"Successfully created: {successful} issues")
