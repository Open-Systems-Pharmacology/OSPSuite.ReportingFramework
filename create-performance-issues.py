...
def get_repository_id(token):
    ...
    if 'errors' in result:
        raise Exception(f"Failed to get repository ID: {result['errors']}")

    return result['data']['repository']['id']


def create_issue(token, repo_id, issue):
    ...
    result = graphql_request(token, mutation, variables)
...
def main():
    if len(sys.argv) < 2:
        ...
        print('3. Select the "repo" scope', file=sys.stderr)
        print('4. Generate and copy the token', file=sys.stderr)
        sys.exit(1)
    ...
    try:
        repo_id = get_repository_id(token)
    ...
    for issue in issues:
        try:
            ...
            result = create_issue(token, repo_id, issue)
