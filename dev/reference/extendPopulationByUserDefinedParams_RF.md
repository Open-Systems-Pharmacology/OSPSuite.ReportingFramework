...
- meanValues:
  ...
  \`parameterPaths\`. The type of mean (arithmetic, geometric) depends
  on the selected \`distribution\`. The values must be in the base units
  of the parameters.
...
- distributions:
  ...
  distributions is defined in \`Distributions\`. Default is
  \`"Normal"\`.
